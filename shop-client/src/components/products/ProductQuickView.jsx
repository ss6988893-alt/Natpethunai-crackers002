...
export default function ProductQuickView({ product, onClose, onAdd }) {
  ...
  useEffect(() => { if (!product) return undefined; setQuantity(1); setSelectedImage(product.image || images[0] || ''); const close = (event) => event.key === 'Escape' && onClose(); document.addEventListener('keydown', close); return () => document.removeEventListener('keydown', close); }, [product, onClose]);
  return <AnimatePresence>{product && <motion.div className="modal-backdrop quick-view-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose}>
    <motion.article role="dialog" aria-modal="true" aria-label={`${product.name} quick view`} className="quick-view" initial={{ scale: .98, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .16, ease: 'easeOut' }} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="quick-view__visual"><img src={selectedImage || product.image} alt={product.name} /><span>{product.priceAvailable !== false ? 'Special price' : 'Price pending'}</span>{images.length > 1 && <div className="quick-view__thumbs" aria-label="Product images">{images.map((image) => <button key={image} className={image === selectedImage ? 'active' : ''} onClick={() => setSelectedImage(image)}><img src={image} alt="" /></button>)}</div>}</div>
      <div className="quick-view__copy"><p className="eyebrow">{product.category}</p><h2>{product.name}</h2>{product.description && <p>{product.description}</p>}<div className="quick-view__price price--deal">{product.priceAvailable !== false ? <><span className="price__change">↓{product.discount}%</span><del>₹{product.originalPrice.toLocaleString('en-IN')}</del><strong>₹{product.price.toLocaleString('en-IN')}</strong></> : <strong>Price on request</strong>}</div>{product.priceAvailable !== false && <div className="quick-view__actions"><div className="quantity"><button onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity"><FiMinus /></button><span>{quantity}</span><button onClick={() => setQuantity(quantity + 1)} aria-label="Increase quantity"><FiPlus /></button></div><button className="button button--gold" onClick={(event) => { flyProductToCart(event.currentTarget.closest('article').querySelector('.quick-view__visual img')); onAdd(product, quantity); onClose(); }}><FiShoppingBag /> Add to cart</button></div>}</div>
    </motion.article>
