...
export default function Products() {
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      gsap.utils.toArray('.product-category-group').forEach((group) => {
        const cards = group.querySelectorAll('[data-product-card]');
        gsap.fromTo(cards, { y: full ? 120 : 55, z: full ? -220 : 0, rotateX: full ? 15 : 0, scale: full ? .84 : .94, opacity: 0, filter: full ? 'blur(10px)' : 'blur(3px)' }, { y: 0, z: 0, rotateX: 0, scale: 1, opacity: 1, filter: 'blur(0px)', duration: .95, stagger: .055, ease: 'power3.out', scrollTrigger: { trigger: group, start: 'top 86%', once: true } });
      });
    }, gridRef);
    return () => context.revert();
  }, [category, search, sort, full, reducedMotion]);
  ...
  return <motion.main id="main-content" className="products-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <motion.div key={category} className="product-scene" initial={{ opacity: .3 }} animate={{ opacity: 1 }} transition={{ duration: .65 }} style={{ '--scene-hue': `${categoryIndex * 24 + 18}deg` }}>
      ...
      <section className="catalog category-catalog container-wide" ref={gridRef}>
