...
async function seed() {
  ...
  const docs = priceListGroups.flatMap((group) => group.items.map(([sourceNumber, name, basePrice], productIndex) => {
    ...
    return { name, slug: slugify(`${name}-${sourceNumber}`, { lower: true, strict: true }), category: categoryBySlug.get(group.slug)._id, description: priceAvailable ? `Official PDF selling price: Rs. ${basePrice}.` : 'Source PDF price was blank. Contact the shop for the current price.', image: '/assets/hero-fireworks.png', basePrice, price: priceAvailable ? basePrice : 0, originalPrice, priceAvailable, sourceNumber, packSize: name.match(/\((?:\d+\s?(?:pc|pcs)|\d+x\d+)\)/i)?.[0] || '', discount: priceAvailable ? Math.round((1 - basePrice / originalPrice) * 100) : 0, status: priceAvailable ? 'in-stock' : 'out-of-stock', featured: priceAvailable && productIndex === 0 };
  }));
