...
export async function login(request, response) {
  ...
  response.setHeader('Set-Cookie', `admin_token=${token}; HttpOnly; Path=/api/admin; Max-Age=${env.jwtCookieSeconds}${cookieSecurity()}`);
  response.json({ success: true, token, data: { id: admin._id, name: admin.name, email: admin.email, role: admin.role } });
}
