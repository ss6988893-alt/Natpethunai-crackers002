...
export async function getProduct(request, response) { const item = await Product.findOne({ isActive: { $ne: false }, $or: [{ slug: request.params.id }, ...(request.params.id.match(/^[a-f\d]{24}$/i) ? [{ _id: request.params.id }] : [])] }).populate('category', 'name slug').lean(); if (!item) return response.status(404).json({ success: false, message: 'Product not found.' }); return response.json({ success: true, data: item }); }
export async function listCategories(request, response) { response.json({ success: true, data: await Category.find({ isActive: { $ne: false } }).sort({ displayOrder: 1, name: 1 }).lean() }); }
